...
| 4 | Audit-log content control: no PHI in telemetry | CC7.2 | §164.312(b), §164.502(b) | 09.aa | **OPEN** — `omatic_search_memory` persists raw query text to `factory.retrieval_events`; no redaction, no retention limit | Data / Carver |
| 5 | Credential protection at rest | CC6.1 | §164.312(a)(2)(iv) | 01.b | **OPEN** — database passwords stored in plaintext in workspace-local `.omatic/factory.json` (mode 0600, gitignored, never committed) | Carver |
...
| 8 | Encryption in transit | CC6.7 | §164.312(e)(1) | 09.y | **MET** — all connections `ssl_mode=verify-full`; certificate chain and hostname verified | Carver |
| 9 | Least privilege / access enforcement | CC6.3 | §164.312(a)(1) | 01.c | **MET** — per-connection `read_write` / `read_only` / `disabled`, enforced at the tool layer before any handler or pool; fails closed on unclassified tools | Carver |
...
**Client identity published** (control 3). Resolved in documentation. Test
fixtures in `omatic-server-connection/scripts/smoke-startup-modes.mjs` still
reference the client name (39 occurrences) and remain to be renamed.

### High

**Query text persisted without limit** (control 4). `omatic_search_memory` writes
the operator's raw query into `factory.retrieval_events` on every call. As of
...
| Transport encryption enforced | `ssl_mode` in every connection; `omatic_list_connections` reports negotiated TLS |
| Access enforcement is real, not advisory | `omatic-server-connection/server/connections.js`, permission chokepoint |
