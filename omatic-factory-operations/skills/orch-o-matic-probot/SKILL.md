...
**This pack ships NO MCP server and NO tools.** That is deliberate — it is why the pack installs on hosts that cannot spawn a local MCP server (Cowork, Claude Code desktop, any sandboxed host), which is the whole reason it exists. There is no `omatic_select_factory`, no `omatic_resolve_factory`, no `omatic_runtime_status`. **Do not call them and do not report their absence as a fault.**

Factory identity comes from the database packet through Conductor (decision #334, halt-rule #288, SOP-021 Step 1). A host with no pin is fully compliant.

*If the separate `omatic-server-connection` MCP plugin also happens to be installed on this host, its resolution tools are a convenience for picking the right connection first on a multi-project machine — routing context, never authority. Nothing below depends on them.*
...
```
STEP 1 — Identify the factory from the DATABASE, not from a file
|- THIS PLUGIN SHIPS NO MCP SERVER AND NO TOOLS. There is no
|    omatic_select_factory and no omatic_resolve_factory here. Their absence is
|    NOT a degraded state and NOT a halt condition — it is the compliant path.
|    Settled by decision #334; active halt-rule #288 and SOP-021 Step 1 both say
|    local configuration is never governance authority. Do NOT read
|    .omatic/factory.json, walk folders, or search local files to start a factory.
|- Call Conductor connections_list.
|    IF Conductor is not present in this host's tool surface -> STEP 5.
|    A missing Conductor is a HOST PAIRING gap, not a factory failure. Say so.
|- Select the granted connection for the factory you were asked to operate,
|    matching on its `database` value, NEVER on a name copied from a document.
|    Connection names are operator-facing strings and go stale silently; the wire
|    does not. Pass the name back VERBATIM, spaces and capitals included.
|    IF that connection is not in the granted set ->
|      Report "BLOCKED — the <x> connection is not granted to this client" and
|      STOP. That is a refusal, never an empty result.
+- -> STEP 2

STEP 2 — Read identity + grant state
|- The startup card in STEP 3 DECLARES the factory identity: read factory_id,
|    corroborated by current_database() and tenant_id. If the declared identity
|    is not the one you intended, that is a MISMATCH TO REPORT — not a file to
|    go consult.
|- pin_state / pin_path: this host has no plugin resolution. Render as
|    "no plugin; factory identified by Conductor connection <name> → <database>".
|    Do NOT render that as BLOCKED or degraded; it is correct and complete.
|- ROSTER HONESTY, and it is not optional: on this host Probot, Fred and Data are
|    loaded as SKILLS with no packaged MCP surface. State that plainly in the
|    report. The card's roster_ready counts Agreements in the database, not what
...
|    pin_state/pin_path      <- "no plugin; factory identified by Conductor
|                               connection <name> → <database>". This host has no
|                               file resolution and needs none. NEVER render it as
|                               BLOCKED — that misreading has cost two sessions.
...
STEP 5 — Conductor unreachable
|- Reached ONLY when Conductor's tools are absent from this host's surface.
|  This is a HOST PAIRING gap, never a factory failure, and the two must not be
|  conflated — that conflation has cost multiple sessions to diagnose.
|- "Probot: BLOCKED — Conductor is not present in this host's tool surface.
|   The factory is not degraded; this host has no route to the broker. Every
|   factory-internal fact is unverified until Conductor is paired here."
|- Do NOT fall back to local files, .omatic/factory.json, or direct PostgreSQL,
|  and do NOT build a startup card from remembered text. Refuse honestly.
|- Remedy to report: Conductor must be registered in THIS host's own MCP config.
|  Registering it for a different surface of the same app does not carry over —
|  measured 2026-08-16, Claude Code and Cowork read different config files and
|  one working registration left the other blind for a day.
...
**A different project or factory** — there is nothing to re-pin on this host.
1. Call `connections_list` and pick the granted connection whose `database` is the target.
2. Name that connection on the `factory_query` call and read the identity the startup card declares.
...
**Advisory mode does not exist in this pack, and its absence must not be improvised into one.** Advisory mode was a state of the *MCP-server* plugin: installed, but its Node runtime failed to resolve, leaving a surface of exactly one tool. This pack ships no server and no runtime, so that state is unreachable here. A tool surface without `omatic_*` tools is the **normal, compliant** state of this pack — never a degradation to declare.

**The state that matters on this host is Conductor reachability**, and it is binary. Conductor present → full operation. Conductor absent → **BLOCKED, host pairing gap**, per STEP 5. Do not diagnose the database, the network, TLS, or credentials when Conductor is simply not registered in this host's MCP config; none of them produce a missing tool surface (KB-0417). Measured 2026-08-16: Conductor was healthy and serving one surface of an app while another surface of the *same app* saw no tools at all, because the two read different config files. Registering it for one does not carry over.
