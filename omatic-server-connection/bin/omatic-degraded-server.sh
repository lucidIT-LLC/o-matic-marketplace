#!/bin/sh
# omatic-degraded-server.sh — the advisory-mode MCP server.
#
...
# diagnostic.
HEADLINE="O-Matic Server is in advisory mode: $CAUSE, so the factory database tools are not available in this session."
...
SKILLS_NOTE="The bundled skills still load and remain usable for planning, routing and advice. They cannot read or write the factory database until the runtime is resolved. Treat any factory-internal fact as unverified while this server is in advisory mode."
...
echo "[omatic-server-connection] advisory mode — $CAUSE" >&2
...
while IFS= read -r line; do
    ...
    case "$line" in
        *'"method":"initialize"'*|*'"method": "initialize"'*)
            ...
            emit "{\"jsonrpc\":\"2.0\",\"id\":${id:-0},\"result\":{\"protocolVersion\":\"$proto\",\"capabilities\":{\"tools\":{}},\"serverInfo\":{\"name\":\"omatic-server-connection\",\"version\":\"$PLUGIN_VERSION-advisory\"},\"instructions\":\"$HEADLINE $SKILLS_NOTE $REMEDY\"}}"
            ;;

        *'"method":"tools/list"'*|*'"method": "tools/list"'*)
            emit "{\"jsonrpc\":\"2.0\",\"id\":${id:-0},\"result\":{\"tools\":[{\"name\":\"omatic_runtime_status\",\"description\":\"$HEADLINE Call this tool for the exact cause and the fix.\",\"inputSchema\":{\"type\":\"object\",\"properties\":{},\"additionalProperties\":false}}]}}"
            ;;

        *'"method":"tools/call"'*|*'"method": "tools/call"'*)
            tool=$(json_string_field name "$line")
            if [ "$tool" = "omatic_runtime_status" ]; then
